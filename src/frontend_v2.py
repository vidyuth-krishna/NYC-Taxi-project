...
current_date_et = pd.Timestamp.now(tz="Etc/UTC") - timedelta(hours=5)
st.title(f"New York Yellow Taxi Cab Demand Next Hour")
st.header(f'{current_date_et.strftime("%Y-%m-%d %H:%M:%S")} (ET)')
...
with st.spinner(text="Fetching batch of inference data"):
    features = load_batch_of_features_from_store(current_date_et)
